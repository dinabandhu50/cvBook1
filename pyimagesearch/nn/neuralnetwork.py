...
class NeuralNetwork:
    def __init__(self, layers, alpha=0.1):
        ...
        # the last two layers are a special case
        # where the input connections need a bias
        # term but the output does not
        w = np.random.randn(layers[-2]+1, layers[-1])
        self.W.append(w / np.sqrt(layers[-2]))

    def __repr__(self):
        # construction and return a string that
    ...
    def sigmoid_deriv(self, x):
        ...
        # ASSUMING that 'x' has already been passed through
        # the 'sigmoid' function
        return x * (1-x)

    def fit(self, X, y, epochs=1000, displayUpdate=100):
        # insert a column of 1's as the feature
        # matrix -- this little trick allows us
        # to treat the bias as a trainable parameter
        # within the weight matrix
        X = np.c_[X, np.ones((X.shape[0]))]

        # loop over the desired number of epochs
        for epoch in range(0, epochs):
            # loop over each indiviadual data point
            #  and train our network on it
    ...
    def fit_partial(self, x, y):
        # construct our list of output activations for
        # each layer as our data point flows through
        # the network; the first activation is a special
